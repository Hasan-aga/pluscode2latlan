...
;(function (root, factory) {
  ...
  var FINAL_LAT_PRECISION_ =
    PAIR_PRECISION_ * Math.pow(GRID_ROWS_, MAX_DIGIT_COUNT_ - PAIR_CODE_LENGTH_)
  ...
  var isValid = (OpenLocationCode.isValid = function (code) {
    ...
    if (code.indexOf(SEPARATOR_) == -1) {
      console.error("Wrong separator index")
      return false
    }
    ...
    if (code.indexOf(PADDING_CHARACTER_) > -1) {
      ...
      if (code.indexOf(SEPARATOR_) < SEPARATOR_POSITION_) {
        console.error("Short codes cannot have padding")
        return false
      }
    ...
    if (code.length - code.indexOf(SEPARATOR_) - 1 == 1) {
      console.error("Wrong length after separator")
      return false
    }
  ...
  var isValidLongitude = (OpenLocationCode.isValidLongitude = function (code) {
    // Work out what the first latitude character indicates for latitude.
    var firstLatValue =
      CODE_ALPHABET_.indexOf(code.charAt(0).toUpperCase()) * ENCODING_BASE_
    if (firstLatValue >= LATITUDE_MAX_ * 2) {
